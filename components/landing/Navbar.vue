<script setup>
const mode = useColorMode();
const open = ref(false);
const menuitems = [
  {
    title: "About",
    path: "/about",
  },
  {
    title: "Contact",
    path: "/contact",
  },
];
const isDark = computed(() => {
  return mode.preference === "dark";
});
const changeTheme = computed(() => {
  mode.preference = mode.value === "dark" ? "light" : "dark";
});
</script>

<template>
  <LandingContainer class="sticky top-0 z-50">
    <header
      class="flex flex-col lg:flex-row justify-between items-center h-full backdrop-blur-sm"
    >
      <div
        class="flex w-full lg:w-auto h-full lg:h-[60px] lg:p-0 py-[30px] px-[10px] items-center justify-between"
      >
        <a href="/" class="text-lg logo">
          <span class="font-bold">JRY</span><span>services</span>
        </a>
        <div class="block lg:hidden">
          <button @click="open = !open" class="text-gray-800">
            <svg
              fill="currentColor"
              class="w-4 h-4"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>Menu</title>
              <path
                v-show="open"
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M18.278 16.864a1 1 0 01-1.414 1.414l-4.829-4.828-4.828 4.828a1 1 0 01-1.414-1.414l4.828-4.829-4.828-4.828a1 1 0 011.414-1.414l4.829 4.828 4.828-4.828a1 1 0 111.414 1.414l-4.828 4.829 4.828 4.828z"
              ></path>
              <path
                v-show="!open"
                fill-rule="evenodd"
                d="M4 5h16a1 1 0 010 2H4a1 1 0 110-2zm0 6h16a1 1 0 010 2H4a1 1 0 010-2zm0 6h16a1 1 0 010 2H4a1 1 0 010-2z"
              ></path>
            </svg>
          </button>
        </div>
      </div>
      <nav
        class="w-full lg:w-auto mt-2 lg:flex lg:mt-0"
        :class="{ block: open, hidden: !open }"
      >
        <ul class="flex flex-col lg:flex-row lg:gap-3">
          <li>
            <a
              href="https://medium.com/@jirayutrpy"
              target="_blank"
              class="flex lg:px-3 py-2"
            >
              Medium
            </a>
          </li>
          <li>
            <a
              href="https://github.com/JIRAYUTRPY"
              target="_blank"
              class="flex lg:px-3 py-2"
            >
              Github
            </a>
          </li>
          <li v-for="item of menuitems">
            <a :href="item.path" class="flex lg:px-3 py-2 a-menu">
              {{ item.title }}
            </a>
          </li>
        </ul>
        <div class="lg:hidden flex items-center mt-3 gap-4">
          <LandingLink href="/contact" styleName="muted" block size="md"
            >Log in</LandingLink
          >
          <!-- <LandingLink href="#" size="md" block>Sign up</LandingLink> -->
        </div>
      </nav>
      <div class="hidden lg:flex items-center gap-4">
        <button class="ml-5 bg-inherit hover:bg-inherit" @click="changeTheme">
          <svg
            v-if="isDark"
            xmlns="http://www.w3.org/2000/svg"
            class="h-6 w-6"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
              d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z"
            />
          </svg>
          <svg
            v-else
            xmlns="http://www.w3.org/2000/svg"
            class="h-6 w-6"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
              fill-rule="evenodd"
              d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z"
              clip-rule="evenodd"
            />
          </svg>
        </button>
      </div>
    </header>
  </LandingContainer>
</template>
<style scoped>
.logo {
  color: var(--color-primary);
}
.logo:hover {
  color: var(--color-secondary);
}
</style>
